...
export const NewOrderModal: React.FC = () => {
  ...
  const handleSelectQuote = (quote: OrcagrafQuote) => {
    ...
    setSellerId(quote.sellerId || null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createManualOrder({
        ...
        sellerCommissionAmountCents: sellerCommissionAmountCents,
        customer: {
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-10 flex flex-col max-h-[90vh]">
        ...
        {creationMode === 'orcagraf' ? (
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {/* Section: Seller & Commission (P2-03) */}
            <div className="bg-slate-50/70 rounded-xl border border-slate-200 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                  <UserCheck className="w-4 h-4 text-sky-600" />
                  <span>Vendedor e Comissão (Opcional)</span>
                </h4>
                {estimatedCommissionCents !== null && estimatedCommissionCents > 0 && (
                  <span className="text-xs font-semibold text-sky-700 bg-sky-50 px-2 py-0.5 rounded border border-sky-200">
                    Comissão estimada: <strong>{formatCentsToBRL(estimatedCommissionCents)}</strong>
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                <div className="sm:col-span-2">
                  ...
                  {orderOrigin === 'ORCAGRAF' && sellerName ? (
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        readOnly
                        value={sellerName}
                        className="w-full text-xs px-3 py-2 bg-slate-100 border border-slate-300 rounded-lg text-slate-800 font-medium cursor-not-allowed"
                      />
                      <span className="text-[10px] uppercase font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 whitespace-nowrap">
                        OrçaGraf
                      </span>
                    </div>
                  ) : (
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (!val) {
                          ...
                          setSellerId(val);
                          setSellerName(user?.name || val);
                        }
                      }}
                      className="w-full text-xs px-3 py-2 bg-white border border-slate-300 rounded-lg focus:ring-1 focus:ring-sky-500 focus:outline-none"
                    >
                      <option value="">Nenhum / Sem vendedor</option>
                      {DEMO_USERS.map((user) => (
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      max="100"
                      placeholder="Ex: 5"
                      value={sellerCommissionPct !== undefined ? String(sellerCommissionPct) : ''}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (val === '') {
                          setSellerCommissionPct(undefined);
                          ...
                          if (!isNaN(num)) setSellerCommissionPct(num);
                        }
                      }}
                      className="w-full text-xs px-3 py-2 pr-7 bg-white border border-slate-300 rounded-lg focus:ring-1 focus:ring-sky-500 focus:outline-none"
                    />
