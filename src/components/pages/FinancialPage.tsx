...
export const FinancialPage: React.FC = () => {
  const { receivables, financialIndicators, registerReceivablePayment } = useArteFlow();
  ...
  const openPayment = (account: ReceivableAccount) => { if (deriveReceivableStatus(account) !== 'PAID') { setSelected(account); setAmount(''); setNotes(''); setError(''); } };
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); if (!selected || submitting) return;
