-- Migration: make order production creation atomic (E9-05)
-- Creates arteflow_create_order_with_production to persist Order, OrderItems, ProductionJobs and Events in a single server-side transaction.
...
begin
  ...
  if not private.arteflow_can_manage_production(p_organization_id) then
