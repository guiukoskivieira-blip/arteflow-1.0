-- Migration: dynamic initial stage resolution per item for order and production jobs (P2-02)
-- Replaces arteflow_create_order_with_production to remove hardcoded stage-entry and use tenant active stages
...
begin
  ...
  -- Resolve default active stage for the organization (is_initial = true first, then lowest sequence)
  select id
  ...
  begin
    insert into public.arteflow_orders (
      ...
      orcagraf_quote_id, delivery_date, created_by, updated_by, data_origin
    ) values (
      ...
      v_clean_quote_id, p_delivery_date, v_user_id, v_user_id, 'user'
...
revoke all on function public.arteflow_create_order_with_production(uuid, text, jsonb, jsonb, text, timestamptz, text) from public, anon;
grant execute on function public.arteflow_create_order_with_production(uuid, text, jsonb, jsonb, text, timestamptz, text) to authenticated;
